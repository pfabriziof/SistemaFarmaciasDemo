...
class ComprobanteController extends Controller
{
    ...
    public function sendMailComprobante(Request $request){
        ...
        //--- Llamar Api PDF/XML comprobante ---
        $invoice = Comprobante::find($request->id);
        ...
        //--- PDF Api Call ---
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $ruta."/print/document/".$invoice->external_id);
        curl_setopt($ch, CURLOPT_HTTPHEADER, array(
            'Content-Type: application/json',
            'Authorization: Bearer ' . $token
        ));
        ...
        //--- XML Api Call ---
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $ruta."/downloads/document/xml/".$invoice->external_id);
        curl_setopt($ch, CURLOPT_HTTPHEADER, array(
            'Content-Type: application/json',
            'Authorization: Bearer ' . $token
        ));
        ...
        EMailer::send('invoice_mail', $request->to_email,
            [
                "to_name"        => $request->to_name,
                "pdf_attachment" => $pdf_response,
                "xml_attachment" => $xml_response,
            ]
        );
