<template>
    <div class="flex-grow-1">
        ...
        <div class="d-flex align-center py-3">
            <h2 style="color: #37474F">Unidades de Medida</h2>
        </div>
        <v-card class="mb-4" light style="padding: 15px">
            <v-row dense class="pa-2 align-center">
                <v-col cols="4">
                    <v-text-field
                        label="Nombre de unidad o código"
        ...
        <v-card class="mb-4" light style="padding: 15px">
            <v-row dense class="pa-2 align-center">
                <v-col v-if="$can('unidadesmed_create', 'all')" class="text-right">
                    <v-btn small color="primary" class="mr-2" @click="addDialog=true;">Agregar Unidad de Medida<v-icon>mdi-plus</v-icon></v-btn>            
                </v-col>
            </v-row>
            <v-data-table
                :headers="headers"
                ...
                <template v-slot:[`item.actions`]="{ item }">
                    <v-btn v-if="$can('unidadesmed_update', 'all')" small icon @click="editReg(item)">
                        <v-icon small> mdi-pencil</v-icon>
                    </v-btn>
                    <v-btn v-if="$can('unidadesmed_delete', 'all')" small icon @click="deleteReg(item)">
        ...
        <v-dialog v-model="addDialog" max-width="45%">
            <v-card>
                <v-card-title>
                    <span class="headline" v-show="!dialogEditar">Agregar Unidad de Medida</span>
                    <span class="headline" v-show="dialogEditar">Editar Unidad de Medida</span>
                </v-card-title>
                <v-card-text>
                    <v-form ref="addForm" @submit.prevent="dialogEditar ? updateReg() : createReg()">
                        <v-row>
                            <v-col>
                                <v-text-field v-model="addForm.unidad_medida" label="Unidad de Medida *" :rules="requiredRules"
                                autocomplete="off"></v-text-field>
                                <v-text-field v-model="addForm.simbolo" label="Símbolo *" :rules="requiredRules"
                                autocomplete="off"></v-text-field>
                                <v-text-field v-model="addForm.codigo_sunat" label="Código de Sunat *" :rules="requiredRules"
                                autocomplete="off"></v-text-field>
                            </v-col>
                        </v-row>
                        <v-card-actions>
...
export default {
    ...
    methods: {
        ...
        getRegistros(page = 1, perPage = 25, sortDesc = 0, sortBy = ""){
            ...
            axios.get('api/unidades_medida?page='+ page +"&perPage="+perPage
                    +"&sortDesc="+sortDesc
                    +"&sortBy="+sortBy
                    +"&"+myParams
            ).then(response => {
        ...
        createReg(){
            ...
            this.addForm.post('api/unidades_medida').then((response)=>{
        ...
        updateReg(){
            ...
            this.addForm.put('api/unidades_medida/'+this.addForm.id_unidad_medida).then((response) => {
                this.addDialog = false;
                this.getRegistros();
        ...
        deleteReg(item){
            this.loadingTable = true;
            if(item.estado!=0){
                var title ='Deshabilitar Unidad de Medida';
                var text = "¿Está seguro que desea deshabilitar esta unidad de medida?";
                var swal_title = 'Deshabilitada!';
                var swal_text = 'La esta unidad de medida ha sido deshabilitada.';
            }else{
                var title ='Habilitar Unidad de Medida';
                var text = "¿Está seguro que desea habilitar esta esta unidad de medida?";
                var swal_title = 'Habilitada!';
                var swal_text = 'La esta unidad de medida ha sido habilitada.';
            }
            Swal.fire({
                title: title,
                text: text,
                ...
                if (result.value == true) {
                    axios.delete('/api/unidades_medida/'+item.id_unidad_medida).then(response => {
