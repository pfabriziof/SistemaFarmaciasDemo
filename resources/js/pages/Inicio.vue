<template>
    <div class="flex-grow-1">
        <div class="d-flex align-center py-3">
            <h2 style="color: #37474F">Bienvenido/a {{usuario.name}} - Dashboard</h2>
        </div>
        <v-row>
            <v-col cols="12" md="6">
        ...
        <v-row>
            ...
            <v-col cols="12" md="6">
                <v-card class="mb-4" light style="padding: 15px">
                    ...
                    <div class="progress-group" v-for="item in stock" :key="item.id_pregunta">
                        {{item.nombreProducto}}
                        {{ item.percentage }}
                        <span class="float-right"><b>{{item.stock_minimo}}</b>/{{item.stock}}</span>
                        <!-- <v-progress-linear color="error" model-value="20"></v-progress-linear> -->
                        <!-- <div class="progress progress-sm">
                            <div class="progress-bar bg-primary" v-bind:style="{width: item.percentage + '%' }"></div>
                        </div> -->
                    </div>
                </v-card>
            </v-col>
...
export default {
    ...
    mounted() {
        ...
        axios.get('api/getDashboardStock').then((response) => {
            for (let index = 0; index < response.data.length; index++) {
                response.data[index].percentage = (response.data[index].stock_minimo*100)/response.data[index].stock;
            }
        ...
        axios.get('api/getDashboardHistoric').then((response) => {
            var labels_chart=[];
            var ventas_chart=[];
            var egresos_chart=[];
            for (let index = 0; index < response.data.length; index++) {
        ...
        axios.get('api/getDashboardToday').then((response) => {
