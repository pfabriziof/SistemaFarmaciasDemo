<template>
    <div class="flex-grow-1">
        ...
        <div class="d-flex align-center py-3">
            <div>
                <h2 style="color: #37474F">{{proveedor.nombre}}</h2> 
        ...
        <v-card class="mb-4" light style="padding: 15px">
            <v-data-table
                ...
                <template v-slot:[`item.add_payment`]="{ item }">
                    <v-btn small v-if="$can('cuentaspagar_update', 'all') && item.total_monto_pendiente > 0" color="success" @click="agregarPago(item)">
            ...
            <v-card>
                ...
                <v-card-text>
                    <v-form ref="form_payment" @submit.prevent="registrarPago()">
                        <v-row>
                            <v-col>
                                <h4>Compra: Factura #{{ form_pago.serie_factura }}-{{ form_pago.nro_factura }}</h4>
                            </v-col>
                        </v-row>
                        <v-row>
                            <v-col cols="4">
...
export default {
    ...
    mounted(){
        this.filter.id_proveedor = this.$route.params.id;
    ...
    methods: {
        ...
        getProveedor(){
            this.preloader = true;
            axios.get('/api/getProveedorDeuda/'+this.filter.id_proveedor).then((response) => {
        ...
        getRegistros(page = 1, per_page = 25){
            ...
            axios.post('/api/getDeudasProveedor?page=' + page,  dataParameters).then(response => {
        ...
        agregarPago(item){
            ...
            this.form_pago.serie_factura     = item.compra.serie_factura;
            this.form_pago.nro_factura       = item.compra.nro_factura;
