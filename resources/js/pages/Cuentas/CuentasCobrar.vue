<template>
    <div class="flex-grow-1">
        ...
        <div class="d-flex align-center py-3">
            <h2 style="color: #37474F">Cuentas Por Cobrar</h2>
        </div>
        ...
        <v-card class="mb-4" light style="padding: 15px">
            ...
            <v-data-table
                ...
                <template v-slot:[`item.nro_comprobante`]="{ item }">
                    {{item.serie}}-{{item.correlativo| zerosPadStart}}
                </template>
                ...
                <template v-slot:[`item.detail_payment`]="{ item }">
                    <v-btn small color="primary" @click="verDetalleDeuda(item)">
            ...
            <v-card>
                ...
                <v-card-text>
                    <v-form ref="form_payment" @submit.prevent="registrarPago()">
                        <v-row>
                            <v-col>
                                <h4>Comprobante: {{form_pago.tipo_comprobante}} #{{form_pago.serie}}-{{form_pago.correlativo | zerosPadStart}}</h4>
                            </v-col>
                        </v-row>
                        <v-row>
                            <v-col cols="4">
                        ...
                        <v-row>
                            ...
                            <v-col cols="4">
                                <v-text-field v-model="form_pago.monto_pago" label="Monto *" placeholder="0,00" type="number" min="0" class="txt-number" :rules="requiredRules" autocomplete="off"></v-text-field>
                            </v-col>
                            <v-col cols="12">
...
export default {
    ...
    methods:{
        ...
        getRegistros(page = 1, perPage = 25, sortDesc = 0, sortBy = ""){
            ...
            axios.get('api/getCuentasCobrar?page='+ page +"&perPage="+perPage
                    +"&sortDesc="+sortDesc
                    +"&sortBy="+sortBy
                    +"&"+myParams
            ).then(response => {
        ...
        getExportData(){
            ...
            this.urltoreturn = '/exportarCuentasCobrar/'+JSON.stringify(exportdata);
        },
        //--- End ---
        ...
        agregarPago(item){
            ...
            this.form_pago.tipo_comprobante = item.tipo_comprobante;
            this.form_pago.serie            = item.serie;
            this.form_pago.correlativo      = item.correlativo;
        ...
        registrarPago(){
            this.preloader = true;
            this.loadingTable = true;
            this.form_pago.post('/api/deuda').then((result)=>{
        ...
        verDetalleDeuda(item, page=1){
            ...
            axios.post('/api/getDeudaPagos?page=' + page, item).then(response => {
                this.deuda_pagos = response.data.data;
                console.log(this.deuda_pagos);
    ...
    watch: {
        dialogPayment(){
            if(!this.dialogPayment){
                // this.$refs.form_payment.reset();
                this.form_pago.tipo_comprobante = '';
                this.form_pago.serie            = '';
                this.form_pago.correlativo      = '';
