<template>
    <div class="flex-grow-1">
        <div class="d-flex align-center py-3">
            <h2 style="color: #37474F">Órdenes de Compra</h2>
        </div>
        ...
        <v-card class="mb-4" light style="padding: 15px">
            <v-data-table
                ...
                <template v-slot:[`item.estado`]="{ item }">
                    <div>
                        <v-chip small v-if="item.estado == 1" class="ma-2" color="success">Aprobada</v-chip>
                        <v-chip small v-if="item.estado == 2" class="ma-2" color="error">Desestimada</v-chip>
                        <v-chip small v-if="item.estado == 3" class="ma-2" color="danger">En Espera</v-chip>
                    </div>
                </template>
                <template v-slot:[`item.actions`]="{ item }">
                    <router-link :to="{name:'OrdenCompraVisualize', params:{id: item.id_orden_compra}}" style="text-decoration: none; color: inherit;">
...
export default {
    data: () => ({
        ...
        headers: [
            ...
            {text: 'Fecha Vencimiento', value: 'fecha_vencimiento', sortable: false},
    ...
    methods:{
        ...
        getRegistros(page = 1, perPage = 25, sortDesc = 0, sortBy = ""){
            ...
            axios.get('api/orden_compra?page='+ page +"&perPage="+perPage
                    +"&sortDesc="+sortDesc
                    +"&sortBy="+sortBy
                    +"&"+myParams
            ).then(response => {
        ...
        verPdf(item){
            window.open('/generarOrdenCompraPDF/'+item.id_orden_compra);
        },
        //--- End ---
